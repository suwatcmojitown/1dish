...
class Guide_model extends CI_Model {
    ...
    public function getContentList($status='',$keysearch='',$active_page=1,$limit)
    {
                ...
                $result = json_decode(callAPI('GET',PATH_API.'backend/guide'.$path.'',''));  
                //console($result);
                if($result->header->res_code=='200')
                {
                        $object = $result;
    ...
    {
            $myJSON = json_encode($data); 
            $result = json_decode(callAPI('DEL',PATH_API.'backend/guide',$myJSON));  
            if($result->header->res_code=='200')
            {
                    return true;
            }
    ...
    {
           $result = json_decode(callAPI('GET',PATH_API.'backend/guide?id='.$id.'',''));  
    ...
    {
            $myJSON = json_encode($data); 
            $result = json_decode(callAPI('PUT',PATH_API.'backend/guide',$myJSON)); 
            //console($result);
